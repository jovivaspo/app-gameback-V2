...
function ModalGame({ category }) {
  ...
  return (
    <>
      <button
        className="button-add text-secondary"
      ...
      >
        <Modal.Header
          closeButton
          className="d-flex justify-content-between bg-dark text-white border border-white"
        >
        ...
        <Modal.Footer className="bg-dark text-white border border-white">
          <AlertMessage />
          <Button variant="outline-light" onClick={() => setModalShow(false)}>
            Close
          </Button>
        </Modal.Footer>
      </Modal>
